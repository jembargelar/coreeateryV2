...
import { ChefHat, ThumbsUp, Flame as SpicyIcon, Star } from 'lucide-react'
...
function DishCard({ item }) {
  return (
    <div className="group rounded-2xl bg-charcoal border border-white/5 overflow-hidden hover:border-gilt/30 transition-colors">
          ...
          <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
            {item.badges.map((b) => {
              ...
              return (
                <span
                  key={b}
                  className={`inline-flex items-center gap-1 rounded-full bg-obsidian/85 backdrop-blur-sm px-2.5 py-1 text-[10px] font-medium ${badgeMeta[b].color}`}
                >
                  <Icon size={11} />
                  {badgeMeta[b].label}
                </span>
      ...
      <div className="p-5">
        <p className="text-[10px] tracking-[0.2em] text-stone uppercase mb-1.5">
          {item.category}
        </p>
        <div className="flex items-start justify-between gap-3">
          <h3 className="font-display font-medium text-lg text-bone leading-snug">
            {item.name}
          </h3>
          <span className="font-body font-semibold text-gilt-soft whitespace-nowrap text-sm mt-1">
            {item.price}
          </span>
        </div>
    ...
    <section id="menu" className="relative bg-obsidian py-24 md:py-32">
      <div className="max-w-7xl mx-auto px-5 md:px-8">
        <div className="flex flex-col items-center text-center mb-14 ring-divider pt-6">
          ...
          <p className="font-body text-stone text-base mt-4 max-w-xl">
            Sebagian dari yang paling dicari tamu kami — harga dalam ribuan
            rupiah (K).
          </p>
        </div>
