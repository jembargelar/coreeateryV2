...
export default function ReviewsSection({ data }) {
  const href = data.cta_url || 'https://www.google.com/maps/search/?api=1&query=COREEATERY+Cianjur'
  return (
    <section className="relative bg-bone text-obsidian py-24 md:py-28">
      <div className="max-w-3xl mx-auto px-5 md:px-8 text-center">
        ...
        <h2 className="font-display font-semibold text-3xl sm:text-4xl text-balance">{data.title}</h2>
        {data.description && (
          <p className="font-body text-obsidian/65 text-base md:text-lg mt-5 leading-relaxed"
            dangerouslySetInnerHTML={{ __html: data.description.replace('ice cream gratis', '<span class="font-semibold text-ember">ice cream gratis</span>') }} />
        )}
        {data.cta_text && (
          <a href={href} target="_blank" rel="noopener noreferrer"
            ...
            <IceCreamCone size={17} />{data.cta_text}
          </a>
        )}
      </div>
    </section>
